...
| `ctx.subagentWorkerRoute` | `core` | [`tool-subagent`](../packages/subagent/tool-subagent) | - | [`tool-subagent`](../packages/subagent/tool-subagent) | - | Owns the settings namespace holding the default child route — provider, model, and reasoning effort — that a delegation tool opting into workerRouteSettings reads on every call, so a settings edit reaches the next delegation without rebuilding a tool definition. |
...
| `ctx.sessionPin` | `core` | [`session-pin`](../packages/session/session-pin) | - | - | - | Appends durable session/pin events and serves the pinned projection list surfaces order by. |
...
| `ctx.remoteTunnel` | `seam` | [`remote-tunnel`](../packages/remote/remote-tunnel) | - | [`remote-access`](../packages/remote/remote-access) | - | Resolves the pinned cloudflared binary and opens one loopback port as a public HTTPS URL; the access service consumes the tunnel lifecycle behind its pairing gate. |
