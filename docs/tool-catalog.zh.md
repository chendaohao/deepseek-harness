...
| `@deepseek-ai/dsh-tool-vision` | `vision_observe` | `ctx.tools`、`ctx.fs`、`ctx.vision`、`ctx.attachments` | `tool/call`、`fs/observed after view presence/absence or successful file operation`、`durable attachment (vision_observe)`、`tool/result` | - | vision_observe 读取本地图片，持久化提交，并从配置的视觉路由返回文本证据——面向纯文本模型路由的工具，与 read_image（要求当前路由接受图片输入）互补。插件仅在 tools/fs/vision/attachments 全部挂载时加载。 |
...
通过独立的视觉模型观察 PNG/JPEG/WebP/GIF 文件并返回文本证据，使纯文本模型路由也能看图。需要配置视觉路由（dsh-vision-llm）。
...
```json
{
  ...
  "properties": {
    ...
    "question": {
      "type": "string",
      "description": "Optional question steering what the vision model should describe."
    }
  },
  "required": [
    "file_path"
...
vision_observe 读取本地图片，持久化提交，并从配置的视觉路由返回文本证据——面向纯文本模型路由的工具，与 read_image（要求当前路由接受图片输入）互补。插件仅在 tools/fs/vision/attachments 全部挂载时加载。
