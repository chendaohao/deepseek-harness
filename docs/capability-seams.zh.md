...
flowchart LR
  ...
  svc_subagentWorkerRoute["ctx.subagentWorkerRoute<br/>Subagent worker-route preference"]
  ...
  svc_sessionPin["ctx.sessionPin<br/>Log-backed session pin state"]
  ...
  svc_remoteTunnel["ctx.remoteTunnel<br/>Public HTTPS tunnel to the local web GUI"]
  ...
  pkg_session_pin --> svc_sessionPin
  ...
  pkg_tool_subagent --> svc_subagentWorkerRoute
  pkg_tools --> svc_tools
  ...
  svc_remoteTunnel --> pkg_remote_access
  ...
  svc_subagentWorkerRoute --> pkg_tool_subagent
...
| `ctx.subagentWorkerRoute` | `core` | [`tool-subagent`](../packages/subagent/tool-subagent) | - | [`tool-subagent`](../packages/subagent/tool-subagent) | - | 拥有保存默认子级路由（provider、model 与 reasoning effort）的设置命名空间；选择启用 workerRouteSettings 的委派工具会在每次调用时读取它，因此设置改动无需重建工具定义即可作用于下一次委派。 |
...
| `ctx.sessionPin` | `core` | [`session-pin`](../packages/session/session-pin) | - | - | - | 追加持久的 session/pin 事件，并对外提供列表界面排序依据的置顶投影。 |
...
| `ctx.remoteTunnel` | `seam` | [`remote-tunnel`](../packages/remote/remote-tunnel) | - | [`remote-access`](../packages/remote/remote-access) | - | 将固定的 cloudflared 二进制解析到本地回环端口，并以公开 HTTPS URL 暴露；访问服务在配对门控之后消费隧道生命周期。 |
