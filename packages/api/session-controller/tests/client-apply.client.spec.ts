...
describe('Session Controller Client apply', () => {
  ...
  it('reopens the control stream immediately on carrier loss instead of waiting for a Host generation', async ({ mock, start }) => {
    const accept = vi.spyOn(ClientSessions.prototype, 'handleControlFrame')
    ...
    await vi.waitFor(() => { expect(client.connection.generation.getSnapshot()).toMatchObject({ id: 2 }) })
  })
})
