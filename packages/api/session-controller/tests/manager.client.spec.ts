...
describe('list lifecycle', () => {
  ...
  it('surfaces a pin from the control projection frame without a list re-pull', async ({ mock, remote }) => {
    remote.session.list.mockResolvedValue(ok({ items: [summary(S1)] as never[] }))
    const manager = makeManager(mock, remote)
    await manager.refreshList()
  ...
  it('clears a stale host-summary pin when the unpin projection frame lands', async ({ mock, remote }) => {
