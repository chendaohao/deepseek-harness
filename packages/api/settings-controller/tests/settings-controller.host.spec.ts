...
describe('settings Remote', () => {
  ...
  it('redacts every described layer and applies mutations through the consumer', async () => {
    ...
    expect(saved.value).toEqual({ provider: 'test', model: 'next', reasoningEfforts: {} })
    ...
    expect((await controller.replace('default-model', {}, undefined)).value)
      .toEqual({ provider: 'test', model: 'original', reasoningEfforts: {} })
  })
...
/** Profile entry the fence tests write to: the fixture's live default model. */
const FORM_NS = 'default-model'
...
describe('the forwarded-write fence on settings writes', () => {
  it('reports writable false and refuses every write for a forwarded request', async () => {
    ...
    for (const call of [
      () => runWithRequestFacts(forwarded(), () => controller.update(FORM_NS, { provider: 'forwarded' }, undefined)),
      () => runWithRequestFacts(forwarded(), () => controller.replace(FORM_NS, { provider: 'forwarded' }, undefined)),
      () => runWithRequestFacts(forwarded(), () => controller.mutate(FORM_NS, [], undefined)),
    ]) {
    ...
    const stored = controller.describe().namespaces.find(candidate => candidate.ns === FORM_NS)
    expect(stored?.value).toMatchObject({ provider: 'test' })
  })

  it('lets forwarded writes through once forwardedWrite is on, and keeps describe truthful', async () => {
    const fixture = await configurationFixture()
    await fixture.ctx.plugin(SettingsController, { forwardedWrite: true })
    const controller = fixture.ctx.settingsController
    const described = runWithRequestFacts({ headers: { 'x-dsh-proxied': '1' } }, () => controller.describe())
    expect(described.writable).toBe(true)
    const written = await runWithRequestFacts(
      { headers: { 'x-dsh-proxied': '1' } },
      () => controller.update(FORM_NS, { provider: 'forwarded' }, undefined),
    )
    expect(written.value).toMatchObject({ provider: 'forwarded' })
  })

  it('treats an in-process call with no request facts as the local case', async () => {
    const { controller } = await boot()
    expect(controller.describe().writable).toBe(true)
    await expect(controller.update(FORM_NS, { provider: 'forwarded' }, undefined)).resolves.toMatchObject({
      value: { provider: 'forwarded' },
    })
  })
