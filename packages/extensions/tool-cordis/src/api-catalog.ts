...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  {
    ...
    methods: [
      ...
      },
      {
        ...
        description: 'Save the complete default model selection. The per-model memory is independent of it, so a selection write restates the remembered choices rather than dropping them. A deployment without a configuration editor keeps its composition entry.',
      ...
      },
      {
        ...
        description: 'Record an explicitly chosen reasoning effort for one model route. A deployment without a configuration editor keeps the composition entry, which carries no memory; the write is then a no-op.',
      ...
      },
      {
        ...
        description: 'Clear one model route\'s remembered effort, for an explicit provider-default choice. A deployment without a configuration editor keeps the composition entry, which carries no memory; the write is then a no-op.',
  ...
  },
  {
    ...
    methods: [
      ...
      {
        signature: 'requestRejection( request: ConnectionTrustRequest, appendHeader?: (name: \'set-cookie\', value: string) => void, ): ConnectionRequestRejection',
        description: 'Apply Connection\'s Host/Origin checks and browser authentication to another Web route.',
        parameters: [{ name: 'request', description: 'request headers from the HTTP or upgrade request.' }, { name: 'appendHeader', description: 'when supplied and the request crossed a UTC day since its cookie was issued, receives a fresh full-lifetime Set-Cookie value to attach to the response; upgrades have no response headers and drop it.' }],
        returns: 'rejection status, or undefined when the route may accept the request.',
      },
      {
        signature: 'admit( request: ConnectionTrustRequest, appendHeader?: (name: \'set-cookie\', value: string) => void, ): PeerAdmission',
    ...
    methods: [
      {
        ...
        description: 'Describe several references for one configuration surface. Batched because a settings page describes every reference its rows name at once, and one round trip keeps those rows from settling separately. A forwarded (paired tunnel) client reads every reference as non-writable: its writes would refuse anyway, so the view never invites a doomed input.',
      ...
      },
      {
        ...
        description: 'Store one value from a configuration surface. The value crosses the wire in this direction only: no read path returns it. Refused unconditionally for a forwarded request — the `forwardedWrite` switch does not reach credentials.',
        parameters: [{ name: 'ref', description: 'reference name to store under.' }, { name: 'value', description: 'the non-empty secret value.' }],
        throws: ['RemoteError when the request is invalid, the request is forwarded, no provider is mounted, or the provider refuses the write.'],
      },
      {
        signature: '@Remote async unset(ref: string): Promise<void>',
        description: 'Remove one reference from a configuration surface. Refused unconditionally for a forwarded request — the `forwardedWrite` switch does not reach credentials.',
  ...
  },
  {
    ...
    methods: [
      ...
      },
      {
        ...
        parameters: [{ name: 'request', description: 'durable address, optionally the last committed sequence the caller already holds.' }, { name: 'signal', description: 'cancellation owned by the Remote stream carrier.' }],
        returns: 'an opening snapshot — complete, or carrying only the events after the requested cursor — followed by gap-free durable event frames and optional cursorless assistant-stream frames.',
      },
      {
    ...
    methods: [
      {
        ...
        parameters: [],
        returns: 'whether this client may write — false for a forwarded request unless {@link Config.forwardedWrite} is on, the client\'s single writable truth — local-document presence, and one view per namespace.',
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  {
    name: 'remote/devices/change',
    mode: 'emit',
    signature: '\'remote/devices/change\'(devices: DeviceView[]): void',
    summary: 'The device roster changed: a device paired, was revoked, or its liveness advanced.',
    description: 'The device roster changed: a device paired, was revoked, or its liveness advanced.',
    parameters: [{ name: 'devices', description: 'the live roster view after the change, each record carrying its window expiry.' }],
  },
  {
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'DeviceView',
    declaration: 'export type DeviceView = DeviceRecord & {\n    expiresAt: number;\n};',
  },
  {
    name: 'DiffCallView',
