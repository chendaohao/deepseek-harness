...
`dsh-remote-access` 让手机或第二台浏览器经 `dsh-remote-tunnel` 打开的隧道配对到宿主 Web GUI，并保持该暴露面被围栏。一行 `dsh web remote: <url>` 加终端二维码完成设备配对（一次性 token，15 分钟 TTL）；配对 cookie 随后把守每个请求与 WebSocket 升级，每日使用滑动前进 30 天，闲置 30 天自动解绑。它为 shell 工具注册 `DSH_REMOTE_URL`，并服务界面内面板驱动的 `/remote/*` 控制面。代理是唯一的公网信任边界：转发流量上现有 `/api` 信任栅栏原样生效。
...
loopback 代理是唯一的公网信任边界：一次性配对 token 铸造的设备 cookie 带 30 天不活跃窗口并随使用滑动（每日 cookie 续期、闲置自动解绑），每个请求与升级都复查绑定，转发请求保持 `/api` 信任栅栏原样生效（盖 `x-dsh-proxied: 1` 标记、剥除浏览器信任头）。隧道下的载体通过 gateway 可配置的 WebSocket Ping 间隔（`websocketHeartbeatIntervalMs`）保活。
