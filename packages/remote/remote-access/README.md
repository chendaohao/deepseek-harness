...
`dsh-remote-access` pairs a phone or second browser to the host's Web GUI over the tunnel `dsh-remote-tunnel` opens, and keeps that exposure fenced. One `dsh web remote: <url>` line plus a terminal QR code pairs a device (one-time token, 15-minute TTL); the paired cookie then gates every request and WebSocket upgrade, sliding forward 30 days on daily use and auto-unbinding after 30 idle days. It registers `DSH_REMOTE_URL` for shell tools and serves the `/remote/*` control plane the in-GUI panel drives. The proxy is the only public trust boundary: forwarded traffic keeps the `/api` trust fence unchanged.
