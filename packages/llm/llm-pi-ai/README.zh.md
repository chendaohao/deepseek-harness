...
询问读 `openai-completions` 与 `openai-responses`——它们「`GET /models` + bearer 认证」的形状是网关、自建服务与官方端点三方一致认可的那一种——以及 `anthropic-messages`：同一路径，但用 `x-api-key` 与 `anthropic-version` 标头，并按 `has_more`/`last_id` 翻页（每页 100 条，最多 20 页）。Azure 尽管出身 OpenAI 也被排除——它用 `api-key` 标头认证并要求 `api-version` 查询参数——Codex 则走 OAuth；其余协议一律以 `DISCOVERY_UNSUPPORTED` 回答，让界面回退到手工填写，而不是把认证失败报成一个没有模型的提供方。`baseURL` 按前缀而非待解析 URL 处理，因此 `https://gateway.example/openai/v1` 这类部署路径会保留其路径段。
