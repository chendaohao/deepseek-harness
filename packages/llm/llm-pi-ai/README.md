...
A catalog model pi-ai marks as non-reasoning — and a hand-declared one with `reasoningEfforts: false` — exposes no `reasoning` at all. pi-ai reports such a model as supporting the single level `off`, but `off` is translated to *omitting* the reasoning option, which is byte-for-byte the request that naming no effort already produces: selecting it could not disable anything, so a provider whose own default is to think would keep thinking with `off` shown as selected. Reporting the capability as unavailable leaves a surface offering the provider's default and nothing that misrepresents it. A hand-declared model that declares no `reasoningEfforts` at all is instead offered the canonical fallback set (`off`, `low`, `high`, `max`), keeping the thinking control usable on a custom vendor that reasons without reporting its levels. The profile `reasoning` value, including `off`, is the deployment default when configured; omitting it preserves the provider default. Per-request `GenerateOptions.reasoningEffort` takes precedence; an effort absent from the exact model capability is normalized by the core to the model's declared default, or dropped when none is declared, rather than failing the request — the adapter still rejects a level it cannot spell on the wire. Describing a model never fails that way: the models under one provider disagree about which levels they accept, so `resolveModel` reports a profile level the exact model cannot take as no default at all rather than throwing. A throw there would take the whole provider out of every model catalog built over it — one mis-set profile field hiding even the models that do support the level — so a bad configuration surfaces where it is acted on, not where it is described. pi-ai's common stream options represent `off` by omitting `reasoning`.
