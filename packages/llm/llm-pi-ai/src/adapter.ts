 ...
 *
 * A model pi-ai marks as non-reasoning — a catalog entry, or a hand-declared
 * one with `reasoningEfforts: false` — is reported by pi-ai as supporting the
 * single level `off`, which is translated to *omitting* the reasoning option:
 * for such a model that is byte-for-byte the same request as naming no effort,
 * so offering it would misrepresent a control that cannot do anything.
 * Omitting `reasoning` entirely is the seam's way of saying the capability is
 * unavailable, which leaves the surface offering only the provider's default.
 * A hand-declared model whose entry declares no `reasoningEfforts` at all
 * carries the canonical fallback set instead (see `resolveModelReasoning`), so
 * its thinking control stays usable on a custom vendor that reasons without
 * reporting its levels.
 * @param model - the resolved model descriptor.
