...
describe('SubagentHeaderLineage', () => {
  ...
  it('toggles the catalog on tap when the pointer cannot hover', () => {
    ...
    // Two idle rows: the trigger names the count the tap opens.
    render(<HeaderCatalog {...props(catalog({ entries: [
      { id: CHILD, mode: 'continuable', label: 'worker', activity: 'inactive' },
      { id: 'child-2' as SessionId, mode: 'one-shot', label: 'reviewer', activity: 'inactive' },
    ] }))} />)
    const trigger = screen.getByRole('button', { name: /2 个子智能体/ })
  ...
  it('leads with active rows and keeps enumeration order within each group', () => {
    ...
    render(<HeaderCatalog {...props(catalog({ entries }), {}, {
      ...
      ...Object.fromEntries(entries.map(entry => [entry.id, {
        id: entry.id, displayTitle: entry.label ?? entry.id, running: entry.activity === 'running',
        blank: false, updatedAt: 1, retainedBy: {},
      }])),
    })} />)

    // Four rows, two of them running: the trigger names the running count.
    hoverCatalog(screen.getByRole('button', { name: '2 个子智能体，正在运行' }))
