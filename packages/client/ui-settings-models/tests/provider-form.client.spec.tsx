...
describe('endpoint interrogation', () => {
  ...
  it('adopts a candidate\'s declared modalities and shows them in the picker', async () => {
    ...
    expandModel(1)
    const imageBox = within(screen.getByRole('group', { name: `${en.modelInputTypes} 1` }))
      .getByRole<HTMLInputElement>('checkbox', { name: en.modelInputImage })
  ...
  it('writes an image claim from the modality checkbox and can retract it to text-only', async () => {
    ...
    // as owning.
    expandModel(1)
    const firstGroup = within(screen.getByRole('group', { name: `${en.modelInputTypes} 1` }))
    expect(firstGroup.getByRole<HTMLInputElement>('checkbox', { name: en.modelInputImage }).checked).toBe(true)
    fireEvent.click(firstGroup.getByRole('checkbox', { name: en.modelInputImage }))

    expandModel(2)
    const secondGroup = within(screen.getByRole('group', { name: `${en.modelInputTypes} 2` }))
    expect(secondGroup.getByRole<HTMLInputElement>('checkbox', { name: en.modelInputImage }).checked).toBe(false)
    fireEvent.click(secondGroup.getByRole('checkbox', { name: en.modelInputImage }))
