...
pi-ai profile 的 `models` 列表就在卡片上编辑：一行一个模型，行上显示 id 与显示名称，上下文窗口与输出上限收在该行的展开区内，右侧是两个无文字的操作——展开与删除。同一展开区还承载该模型的**思考等级**：一行可继承（不声明——手工声明的模型不思考，catalog 模型保留自己的能力）、关闭思考（`false`），或逐级声明提供的档位及其 wire 拼写（一个 dict）。等级词汇像协议选项一样读自所属 namespace 的 schema，保存前由同一套逐行检查把关：声明至少提供一个 `off` 之外的档位，除 `off` 外的每个已声明档位都要填写端点期望的 wire 值——只有 `off` 可以留空，表示支持且不发送参数。新勾选的档位默认以自身作为 wire 值，可按网关另行修改。空列表意味着「使用该路由的内置 catalog」，因此每一行都只会被刻意添加；清空容量会丢弃它，而不是存入一个 schema 会拒绝的值，配置留空的部分由适配器的路由级回退值定尺寸——留空的容量以这些回退值的量级作为占位符，那只是提示而非镜像：该字段按 1000 计 `K`，且部署可以覆盖这些回退值。不是正整数的容量根本不会被存下。
