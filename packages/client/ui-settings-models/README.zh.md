...
每行的名称前有一个拖拽把手。按住把手拖动即可重排行：被拖的行跟随指针，其经过的行向两侧滑开，松手即落位。该顺序是每设备的浏览器偏好——存于 `localStorage` 的 `dsh:provider-order` 键之下，跨页面刷新与 Host 重启保持，且从不写入设置文档，因此配对（转发）客户端可以独立排序自己的行。之后新增的提供方排在已排序行的后面。
