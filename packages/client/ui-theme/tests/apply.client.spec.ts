...
async function bench(
  isLoopback = true,
  describeAnswer: { writable: boolean } = { writable: true },
) {
  ...
  const describe = vi.fn(() => Promise.resolve({
    ok: true as const,
    value: { ...describeAnswer, hasDocument: true, namespaces: [namespace()] },
  }))
...
describe('ui-theme apply', () => {
  ...
  it('loads Host settings at boot, refreshes its namespace, and keeps remote theme writes local', async () => {
    const b = await bench()
    ...
    // A forwarded (paired tunnel) surface reads the same document; its
    // describe reports writable false because the server-side fence refuses
    // its writes.
    const remote = await bench(false, { writable: false })
    ...
    // The in-page switch still applied for this session; only the durable
    // store skipped the doomed write.
