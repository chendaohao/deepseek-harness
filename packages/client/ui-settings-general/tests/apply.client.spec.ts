...
describe('ui-settings-general apply', () => {
  ...
  it('withholds the Host document action off-loopback while the mirror still reads', async ({ mock, start }) => {
    ...
    // The raw settings yaml carries unredacted secrets, so the document action
    // stays loopback-only.
    expect(ownEntries(c, 'settings.action')).toEqual([])
    // The read-only mirror still follows the Host document from any surface:
    // the same boot pair the loopback case reads (the mirror's own `ensure` at
    // apply, then the `connection/reset` of the first connection).
