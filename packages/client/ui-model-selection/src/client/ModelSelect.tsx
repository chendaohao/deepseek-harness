...
export function ModelSelect(
  ...
  // seat (the popupSelect entry does not); both reset each time the menu opens.
  ...
  /** One selectable model row (search hit, recent, or in-group): provider description + trailing check. */
                  ...
                  {searching ? (
                    filteredChoices.length === 0
                      ...
                      : filteredChoices.map(({ group, model, selection }) =>
                        renderModelOption(model, group.name, selection, `${group.id}/${model.id}`))
                  ) : (
                    <>
                        ...
                        return (
                          <section role="group" aria-labelledby={headingId} className={css.group} key={group.id}>
                            ...
                            <div id={modelsId} hidden={isCollapsed}>
                              {group.models.map(model =>
                                renderModelOption(model, model.description, { provider: group.id, model: model.id }, model.id))}
                            </div>
                          </section>
                        )
                      })}
