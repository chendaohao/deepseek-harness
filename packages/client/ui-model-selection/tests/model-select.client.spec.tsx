...
describe('ModelSelect keyboard walk', () => {
  ...
  it('a pane whose rows mark no current value opens on its first row', () => {
    ...
    // No checked row to land on: the keyboard takes the pane's first walkable
    // entry, the provider group header the rows sit under.
    expect(document.activeElement).toBe(screen.getByRole('button', { name: /DeepSeek/ }))
  })
})
