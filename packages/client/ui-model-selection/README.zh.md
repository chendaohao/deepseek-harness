...
模型按提供方分组；当行位于其提供方分组之外（搜索命中与「最近使用」分区）时，行内模型名称下方以次要行显示提供方名称。目录中的说明仍可供其他消费方使用。composer 的模型面板可随提供方增多而扩展：搜索框按模型／提供方名称对目录做扁平过滤，置顶的「最近使用」分区重新提供当前目录仍公布的最近选择（按设备存于 `localStorage`，与移动端面板共享），每个提供方可在自己的表头下折叠。`/model` 弹窗应用所选模型的默认推理强度；composer 随后可以选择任一已公布的推理强度。适配器没有推理元数据时不显示 Effort 行；不存在任意推理强度输入。
