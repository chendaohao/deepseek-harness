 ...
 * @param touchPrimary - whether the input device lacks hover: a gesture-less
 *   editor focus pops the virtual keyboard over a user who only switched
 *   session or loaded the page, so only the reveal runs.
 */
export function focusDraftEditor(editor: LexicalEditor, revealSelection: () => void, touchPrimary: boolean): void {
