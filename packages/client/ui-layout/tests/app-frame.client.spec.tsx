...
describe('AppFrame normal width concessions', () => {
  ...
  it('uses the post-collapse left rail to permit a narrow first opening', () => {
    ...
    const { frame, instance, rightOwner } = mountFrame()
    act(() => { instance.actions.toggleSidebar() })
    // The narrow re-expand floats the panel above the full-width center, so
    // the grid keeps the collapsed rail and the first right-panel eligibility
    // reads the rail-only geometry.
  ...
  it('auto-collapses only below 1024px and preserves the wide sidebar preference', () => {
    ...
    // Crossing below the breakpoint auto-collapses; re-expanding narrow floats
    // the panel over the full-width center (data-sidebar-overlay) without
    // rewriting the stored preference, and re-widening restores it.
    act(() => { instance.actions.toggleSidebar() })
    expect(tracks(frame)).toEqual([SIDEBAR_COLLAPSED, 0])
  ...
  it('re-expands a wide-closed sidebar as a narrow overlay at the contract default', () => {
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.toggleSidebar() })
    resize(980)
    act(() => { instance.actions.toggleSidebar() })
    // The narrow overlay floats above the full-width center at the contract
    // default; the underlying grid keeps the collapsed rail, and the wide
    // preference stays closed so re-widening restores it.
    expect(tracks(frame)).toEqual([SIDEBAR_COLLAPSED, 0])
    expect(frame.hasAttribute('data-sidebar-overlay')).toBe(true)
  ...
  it('narrow toggle re-expands as an overlay over the full-width center and back', () => {
    ...
    expect(frame.hasAttribute('data-sidebar-overlay')).toBe(true)
    expect(slotCalls.findLast(c => c.key === 'sidebar')!.props).toEqual({ collapsed: false, width: 280 })
    expect(frame.querySelector('[data-side="sidebar"]')).toBeNull()
    act(() => { instance.actions.toggleSidebar() })
    expect(tracks(frame)).toEqual([SIDEBAR_COLLAPSED, 0])
    expect(frame.hasAttribute('data-sidebar-overlay')).toBe(false)
