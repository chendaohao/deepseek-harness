...
export function AppFrame({
  ...
  // Narrow overlays: on phones the expanded sidebar floats above the
  // full-width center (with a click-away backdrop) instead of squeezing it
  // past usability — the grid keeps the collapsed rail plus the whole center.
  ...
  const cols = overlaySidebar
    ? computeColumns(viewport, 0, layoutInfo.rightbarTrack ? rightbarPreference : 0, collapsedWidth)
    : computeColumns(viewport, sidebarPreference, layoutInfo.rightbarTrack ? rightbarPreference : 0, collapsedWidth)
  ...
  // rail, so a derived auto-collapse renders the rail UI too). A narrow overlay
  // re-expands the wide content above the full-width center.
  const sidebar = useMemo(() => renderSlot('sidebar', {
    collapsed: sidebarCollapsed,
    width: overlaySidebar ? SIDEBAR_DEFAULT : cols.sidebar,
  }), [renderSlot, sidebarCollapsed, overlaySidebar, cols.sidebar])
  const main = useMemo(() => (
  ...
  return (
    ...
    >
      ...
      {/* Narrow overlay backdrop: one tap outside the floating sidebar closes it. */}
      {overlaySidebar && (
        <div
      ...
      {/* The collapsed rail is fixed-width: no resize handle while closed or
          while the narrow overlay floats (drag handles are not rendered for
          narrow overlays). */}
