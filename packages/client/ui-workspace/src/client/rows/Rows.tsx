 ...
 * @param props.onTogglePin - pin or unpin a session (row menu action; commits without a dialog).
...
export function SessionNodeItem({
  ...
  const sessionMenuItems = [
    ...
    { id: row.pinned ? 'unpin' : 'pin', label: row.pinned ? t('menu.unpin') : t('menu.pin'), icon: <IconPinOutline16 /> },
  ]
        ...
        <span className={css.rowActions}>
          <Menu
            ...
            onSelect={(id) => {
              ...
              if (id === 'pin' || id === 'unpin') onTogglePin(node.id, id === 'pin')
            }}
            portal
            closeOnPointerLeave
            anchor={(
