...
import {
  deriveFlat, deriveGroups, deriveSearchResults, orderByPinTime, orderByRecency, owningGroupKey,
  owningParentFolder, PINNED_KEY, pinCurrentBlank, reconcileManualOrder, UNGROUPED_KEY,
  visibleSessionIds,
} from '../tree.ts'
...
> & {
  ...
  /** Browser-projected order for the pinned section above every Workspace. */
...
function SessionTree({
  ...
  const expandedGroups = useMemo(() => {
    ...
    const expanded = [...workspaces.map(workspace => workspace.workspaceId), UNGROUPED_KEY]
      .filter(key => groupExpansion[key] ?? ancestorKeys.has(key))
    // The pinned section defaults to expanded so pinned sessions stay visible;
    // only an explicit collapse persists.
    if (groupExpansion[PINNED_KEY] ?? true) expanded.push(PINNED_KEY)
    return expanded
  ...
  const groups = useMemo(
    ...
    [list, workspaces, archivedSessionIds, statuses, expandedGroups, ungroupedSessionIds, pinnedOrder],
  )
  useEffect(() => {
  ...
  const commitSessionDrag = (activeDrag: DragState, over: NonNullable<DragState['over']>): void => {
    ...
    // bucket; only real Workspace accounts have a browser-projected order.
...
function FlatList({
  ...
  onSessionTogglePin, usePanelInfo, setSessionOrder,
...
export function WorkspaceBrowser({
  ...
  const pinnedMemberIds = useMemo(
    () => list.ids.filter(id => list.byId[id]?.pinned === true),
    [list],
  )
  ...
  const unpinnedFlatMemberIds = useMemo(
    () => flatMemberIds.filter(id => list.byId[id]?.pinned !== true),
    [flatMemberIds, list.byId],
  )
  ...
  const orderedPinnedSessionIds = useMemo(() => {
    // The pinned section keeps a browser-local manual order; its recency
    // fallback is pin time (newest pin first), not activity time.
    const baseOrder = orderBy === 'updated'
      ? orderByPinTime(pinnedMemberIds, list.byId)
      : reconcileManualOrder(pinnedMemberIds, sessionOrderByAccount[PINNED_KEY], list.byId)
    return pinCurrentBlank(
      baseOrder,
      currentBlank !== undefined && pinnedMemberIds.includes(currentBlank) ? currentBlank : undefined,
    )
  }, [currentBlank, list.byId, orderBy, pinnedMemberIds, sessionOrderByAccount])
  const orderedFlatSessionIds = useMemo(() => {
    const baseOrder = orderBy === 'updated'
      ? [...orderByPinTime(pinnedMemberIds, list.byId), ...orderByRecency(unpinnedFlatMemberIds, list.byId)]
  ...
  }, [currentBlank, flatMemberIds, list.byId, orderBy, pinnedMemberIds, sessionOrderByAccount, unpinnedFlatMemberIds])
  const activeSessionOrders = useMemo<Readonly<Record<string, readonly string[]>>>(() => Object.fromEntries([
    ...
    [PINNED_KEY, orderedPinnedSessionIds] as const,
    [UNGROUPED_KEY, orderedUngroupedSessionIds] as const,
    [FLAT_SESSION_ORDER_KEY, orderedFlatSessionIds] as const,
  ]), [orderedFlatSessionIds, orderedPinnedSessionIds, orderedUngroupedSessionIds, orderedWorkspaces])
  useEffect(() => {
          ...
          : groupBy === 'flat'
            ...
            : (
              <SessionTree
                ...
                pinnedOrder={orderedPinnedSessionIds}
