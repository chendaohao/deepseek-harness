...
describe('deriveGroups', () => {
  ...
  it('marks the pinned section current when the selected session is pinned', () => {
    ...
    const groups = deriveGroups(
      withMain(list(pinned), pinned.id), [ws], noArchive, noAttention,
...
describe('deriveFlat', () => {
  ...
  it('orders pinned members newest pin first, then by recency', () => {
    const pinnedOld = { ...summary('pinned-old', 5), pinned: true, pinAt: 50 }
    const pinnedNew = { ...summary('pinned-new', 1), pinned: true, pinAt: 90 }
    const recent = summary('recent', 30)
    const ids = orderByPinTime([pinnedOld.id, pinnedNew.id], list(recent, pinnedOld, pinnedNew).byId)
    expect(ids).toEqual([sid('pinned-new'), sid('pinned-old')])
  })
  ...
  it('treats a missing pinAt as the epoch and falls through equal pins to recency', () => {
    ...
    const pinnedEqualC = { ...summary('pinned-equal-c', 50), pinned: true, pinAt: 90 }
    // Alternating input exercises both pin ranking directions, the missing
    // pinAt falls back to the epoch from either side, and pinAt ties fall
    // through to the updatedAt order.
    const members = [pinnedNoPinAt.id, pinnedEqualA.id, pinnedEqualC.id, pinnedEqualB.id]
    const byId = list(pinnedNoPinAt, pinnedEqualA, pinnedEqualB, pinnedEqualC).byId
    const expected = [sid('pinned-equal-c'), sid('pinned-equal-a'), sid('pinned-equal-b'), sid('pinned-no-pin')]
    expect(orderByPinTime(members, byId)).toEqual(expected)
    expect(orderByPinTime([...members].reverse(), byId)).toEqual(expected)
  })

  it('renders only the supplied flat member ids, pinned and ordinary alike', () => {
    const pinned = { ...summary('pinned', 5), pinned: true, pinAt: 90 }
    const recent = summary('recent', 30)
    const rows = deriveFlat(list(recent, pinned), [pinned.id, recent.id], noAttention)
    expect(rows.map(row => row.id)).toEqual([sid('pinned'), sid('recent')])
