...
describe('workspace browser rows', () => {
  ...
  it('reveals a clipped session title by scrolling it while the row is hovered', () => {
    const node: SessionNode = {
      id: sid('clipped'), title: 'A Session Title Long Enough To Be Clipped (1)', blank: false, pinned: false,
  ...
  it('returns a revealed title to its start in one step', () => {
    const node: SessionNode = {
      id: sid('instant-return'), title: 'A Session Title Long Enough To Be Clipped (1)', blank: false, pinned: false,
      running: false, runningSubagentCount: 0, completed: false, hasActiveSchedule: false, updatedAt: 0,
    }
    render(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
  ...
  it('pinned bucket shows the pinned label and no workspace actions', () => {
    const group: GroupNode = {
      ...
      sessions: [{
        ...
        runningSubagentCount: 0, completed: false, pinned: true, pinAt: 9, hasActiveSchedule: false, updatedAt: 0,
      }],
    }
    render(<ProjectRowItem group={group} onToggle={vi.fn()} onCreate={vi.fn()} t={t} />)
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, archive, and pin', () => {
    const onOpen = vi.fn()
    const onRename = vi.fn()
    const onFork = vi.fn()
    ...
    expect(onRename).toHaveBeenCalledOnce()
    expect(onOpen).not.toHaveBeenCalled()
    // Unpinned rows offer 置顶会话 and dispatch pinned=true.
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
  ...
  it('pinned rows offer 取消置顶 and dispatch pinned=false', () => {
    const onTogglePin = vi.fn()
    const node: SessionNode = {
      id: sid('pinned'), title: 'Pinned', blank: false, running: false,
      runningSubagentCount: 0, completed: false, pinned: true, pinAt: 9, hasActiveSchedule: false, updatedAt: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
    ...
    expect(onTogglePin).toHaveBeenCalledWith(node.id, false)
  })
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, runningSubagentCount: 0, completed: false, pinned: false, hasActiveSchedule: false, updatedAt: 0,
      }
