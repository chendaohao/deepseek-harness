...
export class HostConnectionService extends Service implements HostConnectionHandle {
  ...
  admit(
    ...
    const rejection = this.requestRejection(request, appendHeader)
  ...
  private register(
    ...
    const fetchHandler = rpcFetchHandler(channel, handler, this.operator, this.ctx.logger)
    const route: WebRoute = {
      kind: 'prefix',
      path: channel,
      handler: async (req, res) => {
        const admission = this.admit(req, (name, value) => { res.appendHeader(name, value) })
  ...
  private registerInterceptor(
    ...
    const interceptor: ConnectionRpcInterceptor = {
      matches,
      fetchHandler: rpcFetchHandler(channel, handler, this.operator, this.ctx.logger),
    }
    return owner.effect(() => {
