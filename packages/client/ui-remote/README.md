...
**Desktop panel** — a phone-glyph trigger beside Settings in the sidebar foot (`sidebar.footer.action`) opens a modal that shows the tunnel status badge, the one-time pairing QR, the paired-device roster with per-device rename and revocation, and the stop-all action. Data rides the desktop-only remote-access control plane — `GET /remote/state`, `POST /remote/pair/issue`, `POST /remote/devices/<deviceId>/revoke`, `POST /remote/devices/<deviceId>/rename`, and `POST /remote/stop` — plus the forwarded `remote/devices/change` and `remote-tunnel/state` events while the panel is open.
