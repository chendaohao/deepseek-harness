...
**桌面面板** —— 侧栏底部设置旁的手机图标入口（`sidebar.footer.action`）打开一个 modal，展示隧道状态徽标、一次性配对二维码、带逐设备吊销的已配对设备列表，以及停止全部操作。数据走仅桌面的 remote-access 控制面——`GET /remote/state`、`POST /remote/pair/issue`、`POST /remote/devices/<deviceId>/revoke`、`POST /remote/stop`——以及面板打开期间转发的 `remote/devices/change` 与 `remote-tunnel/state` 事件。
