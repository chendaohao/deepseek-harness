...
describe('HarnessSdkJsonRpcServer', () => {
  ...
  it('normalizes an unsupported reasoning effort during initialize', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'dsh-jsonrpc-unsupported-reasoning-'))
    const ctx = await makeHarness(storageDir)
    vi.stubEnv('DEEPSEEK_API_KEY', 'test-key')
    try {
      const server = new HarnessSdkJsonRpcServer(ctx, new FakeTransport())
      // An effort the model does not declare is normalized by the core (its
      // declared default, or dropped when none is declared) rather than
      // rejected, so the session still initializes.
      ...
      expect((server as unknown as { initialized: boolean }).initialized).toBe(true)
      await server.shutdown()
    } finally {
      await ctx.fiber.dispose()
      await rm(storageDir, { recursive: true, force: true })
