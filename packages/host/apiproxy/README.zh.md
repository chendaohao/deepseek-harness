...
`session.selectModel` 会把接受的切换保存为部署默认值；没有单独的选择动作。它存储已解析的 `ModelSelection`，包括适配器实体化的默认推理（reasoning）强度。不带等级的选择是普通模型切换，会先按该路由记忆的等级解析——用户在该精确路由上显式选择过、且模型仍提供它——再回退到适配器默认；显式等级会被记忆，显式「提供方默认」选择（不带等级、`reasoningEffortExplicit: true`）会清除记忆。完整分节写入会在所选模型没有推理强度时清除已存的默认值，而按模型记忆不受影响。存储失败只记日志，不会撤销会话选择。没有设置提供方的部署保留组合条目，切换只对当前会话生效。
...
`session.history` 会读取已附加 Session 的内存状态，或通过持久化检查冷日志，而不会恢复或发布 agent，然后按追加来源的消息边界分页：`maxMessages` 统计以追加方式进入 surface 的 `user/message` 和 `assistant/message` 事件，因此仅供模型使用的替换副本不占用配额。每一页仍是一段连续的原始事件区间，从而让压缩（compaction）的仅日志 `compaction/summary` 记录与引用它的替换留在同一页。页面载荷随会话内容增长，因此 `session.history`（以及 `subagent.history`）可以选择退出默认 30 秒 unary 截止：Web 载体选择退出，其运行时调用方信号——连接 generation 中止加宽裕页面上限——才是真正的截止；不传信号的载体（mobile core）保持默认预算，而调用方中止始终会取消客户端侧 fetch。
