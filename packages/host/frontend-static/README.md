...
Cache headers: Vite content-addressed assets (`assets/*`, fonts, langs — the name embeds the content hash) ship `Cache-Control: public, max-age=31536000, immutable`; other static files get `public, max-age=3600, must-revalidate` plus a sha1 ETag, except the service-worker script which is `no-cache` so deployed workers take over on the next visit; every index response is `no-cache` with an ETag and answers 304 on a matching `If-None-Match`, because the index-tap boot manifest changes with the plugin set.
