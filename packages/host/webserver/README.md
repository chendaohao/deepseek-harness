...
- **Compression covers text bodies** — `compression: 'auto'` (default) negotiates brotli/gzip per request over the compressible MIME set with a 1 KiB threshold; SSE passes through uncompressed, and `compression: 'none'` disables the pass entirely (reverse-proxy deployments that compress themselves). A compressed response drops `content-length` (chunked framing) and adds `Vary: accept-encoding`.
- **HTTP/1.1 only** — the carrier is a `node:http` server; HTTP/2 multiplexing requires fronting it with a reverse proxy (the remote-access proxy or any h2 terminator).
