...
连接失败只记日志、绝不致命：清单会引导代理回退到 `codegraph explore` CLI，并且失败的连接会在下一个会话的第一个带索引 pre-step 被丢弃并重启，因此一次暂时的 codegraph daemon 故障不会让 MCP 工具在整个 web 生命周期内失效。
...
带索引的工作区、首个回合：会话携带一条额外的用户消息（CodeGraph 清单），工具目录在惰性连接就绪后包含 `mcp__codegraph__*`。在此之前（或启动失败时），清单会引导模型使用 `codegraph explore` shell fallback。非索引工作区：无消息、无服务器、无工具。
...
- **MCP 启动尽力而为** —— 服务器在第一个带索引的 pre-step 异步启动；连接就绪前首个回合的工具目录可能缺少这些工具，启动失败时会话只能使用 shell fallback（清单对此有明确说明）。连接会在之后每个会话中重启一次，直到成功或插件重载。
