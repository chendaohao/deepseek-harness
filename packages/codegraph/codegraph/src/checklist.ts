...
export const CODEGRAPH_CHECKLIST_BODY: string = [
  ...
  '2. 若存在 → 优先调用会话工具中的 `mcp__codegraph__*`（本插件已挂载 codegraph MCP server）：',
  '   - **每次调用都传 `projectPath: <当前工作区绝对路径>`**（DSH 的 MCP 连接不带 rootUri、服务端无默认项目；传了 projectPath 即可打开任意已索引项目）',
  '   - 首选 `mcp__codegraph__codegraph_explore`（查结构/调用链/影响面，一次调用返回源码+路径），需要深度时用 `mcp__codegraph__codegraph_node`，全文检索用 `mcp__codegraph__codegraph_search`',
  ...
  '',
  '- **Preferred MCP tools** (mounted by the codegraph plugin):',
  ...
  '- **Shell fallback**: `codegraph explore \\\\"<symbol names or question>\\\\"` — use only if the MCP tools are unavailable.',
  '',
