...
;(subjectBuilt ? describe : describe.skip)('packed image', () => {
  it('materializes the roster with every dependency resolved', () => {
    ...
    // Third-party runtime JavaScript ships as published files inside the
    // image (the current webserver closure: schemastery's own deps), not as
    // bundled code.
    expect(Object.hasOwn(result.files, 'node_modules/js-yaml/dist/js-yaml.mjs')).toBe(true)
    expect(Object.hasOwn(result.files, 'node_modules/@standard-schema/spec/package.json')).toBe(true)
  })
