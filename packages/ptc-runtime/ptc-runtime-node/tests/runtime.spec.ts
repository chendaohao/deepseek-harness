...
describe('Node program process — strip-failure localization', () => {
  it('points at the line whose nested quoting breaks the parse', async () => {
    ...
    const result = await run({
      ...
      bindings: bindings({ bash: async () => ({ stdout: { text: '' } }) }),
    })
    expect(result.error?.kind).toBe('exception')
  ...
  it('localizes a single-line program to line 1', async () => {
    const { run } = await setup()
    const result = await run({ program: 'const x = { a: "b };', bindings: [] })
    expect(result.error?.kind).toBe('exception')
  ...
  it('leaves in-worker thrown errors unlocalized', async () => {
    const { run } = await setup()
    const result = await run({ program: 'throw new Error("boom")', bindings: [] })
    expect(result.error?.kind).toBe('exception')
