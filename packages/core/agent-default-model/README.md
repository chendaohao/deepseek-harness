...
`currentSelection()` returns a detached selection. A captured selection stays stable while later operations read updated Config references. `saveSelection()` writes the whole selection through the profile-backed configuration editor when one is composed, and the three effort-memory methods read or patch the `reasoningEfforts` map in the same section. `saveSelection()` rebuilds the section wholesale, so it carries the current map along to keep the per-model memory intact.
