...
describe('dsh-tool-subagent model selection', () => {
  ...
  it('uses the LLM runtime for provider validation and effort normalization before child creation', async () => {
    let starts = 0
    const ctx = await setup({ provider: 'mock', withModelSelection: true }, { onStart: () => { starts += 1 } })
    ctx.llm.registerAdapter(['alpha'], new MockAdapter([], REASONING))

    // An effort the model does not declare is normalized by the core to the
    // model's declared default rather than rejected, so the child starts.
    const normalized = await callSubagent(ctx, {
      description: 'undeclared effort',
      prompt: 'do it',
      provider: 'alpha',
      model: 'fast-model',
  ...
  it('normalizes a configured effort the exact model cannot take before child creation', async () => {
    let starts = 0
    const ctx = await setup({
      provider: 'mock',
      agentOptions: {
    ...
    // The configured 'high' is not declared: the core normalizes it to the
    // model's declared default ('low') instead of failing the request.
