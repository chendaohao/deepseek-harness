...
Alpha 迁移边会拒绝冻结清单之外的所有事件类型，包括带有 `ignorable: true` 标记的未知事件。它也会拒绝意外的 payload 成员。自带版本号的嵌套成员，其接纳范围由**已发布写入端可能产出过的版本**决定，而不是安装态包的当前值：`subagent/descriptor` 接纳版本 2 与 3，因为 `SUBAGENT_DESCRIPTOR_VERSION` 在格式 v0 仍为当前写入端的期间走过了这两个版本，而版本 1 早于首个发布。`tool/result.meta` 与嵌套 PTC `arguments` 是显式的不透明 JSON 字段；迁移会原样保留它们，不把其中的数字解释为会话序号。内容块中未知的 `type` 分支、消息来源中未知的 `kind` 分支、assistant 结束原因中未知的 `kind` 分支与 `turn/end` 原因中未知的 `kind` 分支保持 owner-opaque JSON，已知分支则接受结构校验。
