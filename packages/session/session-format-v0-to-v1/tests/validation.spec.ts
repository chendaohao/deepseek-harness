...
const validPayloads: Readonly<Record<string, SessionFormatJsonValue>> = {
  ...
  'feedback/message-put': {
    ...
    item: { messageId: 'assistant-1', rating: 'positive', version: 'v1', createdAt: 1, updatedAt: 1 },
  },
  'feedback/message-delete': { sessionId: 'feedback-session', messageId: 'assistant-1' },
  'goal/change': {
...
describe('released event and payload inventory', () => {
  it('has an executable valid fixture for every frozen released-v0 event type', () => {
    ...
    expect(KNOWN_SESSION_EVENT_TYPES.has('session/pin')).toBe(true)
    expect(KNOWN_SESSION_EVENT_TYPES.has('feedback/message-put')).toBe(true)
    expect(KNOWN_SESSION_EVENT_TYPES.has('feedback/message-delete')).toBe(true)
