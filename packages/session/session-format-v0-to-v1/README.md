...
The alpha edge refuses every event type outside its frozen inventory, including an unknown event marked `ignorable: true`. It also refuses unexpected payload members. A nested member that carries its own version is admitted by the versions a released writer could have emitted, not by the installed package's current value: `subagent/descriptor` accepts versions 2 and 3, because `SUBAGENT_DESCRIPTOR_VERSION` advanced through both while format v0 was the current writer, and version 1 predates the first release. `tool/result.meta` and nested PTC `arguments` remain explicit opaque JSON fields and are preserved without Session-sequence interpretation. Unknown content-block `type`, message-source `kind`, assistant finish-reason `kind`, and `turn/end` reason `kind` arms remain owner-opaque JSON while their known arms receive structural validation.
