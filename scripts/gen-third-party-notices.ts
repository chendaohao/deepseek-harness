...
export function virtualManifest(virtual: string, name: string): VirtualManifest | undefined {
  ...
  if (entry !== undefined) {
    // The store keeps a directory for every lockfile-referenced version even
    // when pnpm skipped installing a platform-specific optional payload; the
    // content scan below is the fallback for those.
