...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/remote/remote-access': { kind: 'indirect', reason: 'The pairing gate and proxy register only the DSH_REMOTE_URL shell-env contribution; the shell tools own what reaches a model request.' },
  'packages/remote/remote-tunnel': { kind: 'none', reason: 'The package owns tunnel transport only; no URL, ticket, or cookie reaches a model request.' },
  ...
  'packages/client/ui-remote': { kind: 'none', reason: 'Browser-side remote-control surface renders logged state over the /api wire and never assembles or sends a provider request itself.' },
  ...
  'packages/bundle/workspace-ext': { kind: 'indirect', reason: 'The bundle is a patch-list carrier; each inserted row\'s package owns its model-facing behavior.' },
