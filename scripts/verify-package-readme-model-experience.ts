...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/vision/vision': { kind: 'indirect', reason: 'The definition package delegates all model rendering to its providers and consumers.' },
  ...
  'packages/client/ui-settings-remote': { kind: 'none', reason: 'Browser-side Settings page over the remote-access device registry; registers nothing model-facing.' },
  ...
  'packages/remote/remote-tunnel': { kind: 'none', reason: 'The tunnel provider owns transport only; no URL, ticket, or cookie reaches a model request.' },
  'packages/remote/remote-access': { kind: 'indirect', reason: 'The package registers the DSH_REMOTE_URL shell-env fact; the shell tools own what reaches a model request.' },
