...
- 按住麦克风说话，松开发送；agent 工作期间点按麦克风 = 停止朗读并取消回合（打断）。
- 连续聆听（每轮回复结束后自动开始聆听）是设置项之一。
- 回复流式进入聊天并按句朗读；代码块永不朗读。回复以 Markdown 渲染（代码块、列表、链接），工具活动以可展开的行内条目出现在对话流中。
- 可从相机或相册发送图片；agent 连同文字一起看到，消息行内渲染图片。
- 长按任意消息可复制或分享；快捷指令 chips 提供一键开头的提示词。
- 审批、agent 提问、计划模式横幅与待办列表以内联面板呈现。
- ☰ 按钮列出主机会话（切换或新建）；设置里可选会话模型、识别与朗读语言、朗读语速与音调、自动朗读、连续聆听。
- 界面文案跟随设备语言（中文/English）；暗色模式跟随系统主题；App 图标为黑底 DeepSeek 鲸鱼标。
