...
- **apps/mobile**（`@deepseek-ai/dsh-mobile-app`，private），Expo SDK 57 壳。配对 = 扫描终端二维码 → `pairWithHost`（`redirect: 'manual'`）→ `dsh_remote` cookie 存入 `expo-secure-store`（钥匙串）。适配器：`expo-speech-recognition`（设备端 ASR）、`expo-speech`（设备端 TTS）、`expo/fetch`。屏幕：配对（扫码或手输 URL）、聊天（消息、工具条、麦克风、审批/提问卡片、设置）。
