...
- **Device-scoped cookie v2** (`dsh_remote = v2.<deviceId>.<expiresDay>.<mac>`): the HMAC binds the device id, and `authorize` requires the device to still be **live** in the persisted roster (`$DSH_HOME/secrets/remote-devices.json`, 0600). Revocation deletes the record, so the next request answers 401 — no rotation of the master secret needed.
